       ...
       box-border w-[100%] h-[100%] bg-[#fff]">
        <div class="text-[14px]" style="user-select: none;">
            ...
            <RadioGroup v-model:value="dateType" @change="handleDateTypeChange">
                <RadioButton v-for="item in dateTypes" 
                :key="item.value" :value="item.value">{{ item.label }}</RadioButton>
            </RadioGroup>
            <RangePicker v-model:value="dateRangeValue" @change="handleSearch"
             class="ml-[10px]" v-if="dateType === ''" />
            <DatePicker v-model:value="dateValue" @change="handleSearch"
            :picker="pickerType" class="ml-[10px]"
            value-format="YYYY-MM-DD HH:mm:ss"
            v-else />
            <span class="ml-[10px]">区域选择：</span>
            <TreeSelect v-model:value="selectedRegionKey" 
            :style="{ maxHeight: '400px', width:'240px'}"
            allow-clear
            :field-names="{
                children: 'children', label: 'region_name', value: 'region_id'
            }"
            show-search
            tree-node-filter-prop="region_name"
        ...
        <div class="report-list flex-1 overflow-hidden flex border-[1px] border-[#E5E5E5] mt-[10px]">
            ...
            <div class="table-view h-full overflow-y-auto bg-[#F8FAFC]"
             style="flex:0.6">
...
import { RadioGroup,RadioButton,DatePicker,RangePicker,TreeSelect,Modal,message } from 'ant-design-vue';
